...
//2) показать должников с адресами электронной почты в доменах rhyta.com или dayrep.com.
var query2 = debtors.Where(d => d.Email.EndsWith("rhyta.com") || d.Email.EndsWith("dayrep.com")).ToList();
//3) показать должников в возрасте от 26 до 36 лет.
var query3 = debtors.Where(d => (DateTime.Now - d.BirthDay).Days >= 25 * 365 && (DateTime.Now - d.BirthDay).Days < 36 * 365).ToList();
//4) показать должников задолженность которых не превышает 5000.
var query4 = debtors.Where(d => d.Debt <= 5000).ToList();
//5) показать должников у которых полное имя состоит из более 18 символов и номер телефона состоит из 2 и более  цифр 7.
var query5 = debtors.Where(d => d.FullName.Length > 18 && d.Phone.Count(c => c == '7') >= 2).ToList();
//7) показать должников рожденных зимой.
var query7 = debtors.Where(d => d.BirthDay.Month == 12 || d.BirthDay.Month == 1 || d.BirthDay.Month == 2);
//8) показать должников у которых долг превышает средний долг общих долгов, и отсортировать их сначала по фамилии, а затем по убыванию суммы.
var query8 = debtors.Where(d => d.Debt > debtors.Average(d => d.Debt)).OrderBy(d => d.FullName.Split(" ")[2][0]).ThenByDescending(d => d.Debt).ToList();
//9) показать фамилии,  возраст и общей суммы долга должников, где в номере телефона нет цифры 8
var query9 = debtors.Where(d => !d.Phone.Contains('8')).Select(d => d.FullName.Split(" ")[2] + " " + ((DateTime.Now - d.BirthDay).Days / 365).ToString()).ToList();
query9.Add(debtors.Where(d => !d.Phone.Contains('8')).ToList().Sum(d => d.Debt).ToString());
//11) Составьте список должников, у которых в имени и фамилии есть не менее 3 одинаковых букв, и отсортируйте их по алфавиту.
var query11 = debtors.Where(d => (d.FullName.Split(" ")[0] + d.FullName.Split(" ")[2]).GroupBy(c => c).Any(g => g.Count() >= 3)).OrderBy(d => d.FullName).ToList();
//13) показать год, в котором родилось наибольшее количество должников
var query13 = new List<int>() { debtors.GroupBy(d => d.BirthDay.Year).Where(g => g.Count() == debtors.GroupBy(d => d.BirthDay.Year).Max(g => g.Count())).ToList()[0].Key };
//14) показать 5 должников с самым большим долгом.
var query14 = debtors.OrderByDescending(d => d.Debt).Take(5).ToList();
//15) показать общую сумму долгов 
var query15 = new List<int>() { debtors.Sum(d => d.Debt) };
//16) показать список должников, увидевших II мировую войну.
var query16 = debtors.Where(d => d.BirthDay >= new DateTime(1939, 9, 1) && d.BirthDay <= new DateTime(1945, 9, 2)).ToList();
//17) показать список должников, от имени и фамилии которых можно написать слово «smile».
var query17 = debtors.Where(d => (d.FullName.Split(" ")[0] + d.FullName.Split(" ")[2]).ToLower().Contains('s') && (d.FullName.Split(" ")[0] + d.FullName.Split(" ")[2]).ToLower().Contains('m') && (d.FullName.Split(" ")[0] + d.FullName.Split(" ")[2]).ToLower().Contains('i') && (d.FullName.Split(" ")[0] + d.FullName.Split(" ")[2]).ToLower().Contains('l') && (d.FullName.Split(" ")[0] + d.FullName.Split(" ")[2]).ToLower().Contains('e')).ToList();
